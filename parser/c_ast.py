"""parser/c_ast.py - Parse for C files."""  # noqa: INP001
from globalstuff import G, COLOR, REF_ROOT, REF_OLD, REF_POS, REF_C_AST, FILE_ERROR
...
def process_c_ast(CS: ChangeSetType) -> None:
    """Generate the AST and extract it."""
    am = Ast_Manager(CS)
...
def get_prior_tags(CS: ChangeSetType) -> None:
    """Get tags from the prior version."""
    with CS(REF_OLD):
...
def close_prior_tags(CS: ChangeSetType) -> None:
    """Close tags from the prior version."""
    if CS.prior_tags:
        with CS(REF_OLD):
...
class Ast:
    ...
    def ast_debug(self, CS: ChangeSetType, ast_view_pos: int) -> None:
        """Add CS infos to m_ast_debug."""
        CS.store(m_ast_debug.set(
            CS.ref(m_ast.ast_id, REF_POS, ast_view_pos),
            json.dumps(self.__dict__, default=serializer),
        ))
        return

    def tag(self, CS: ChangeSetType, ast_view_pos: int, line: Line|None=None) -> None:
    ...
    def extract_1arg(self, CS: ChangeSetType, type_id: int, name: int|str, line: Line|None=None) -> None:
        """m_ast only handler."""
        ast_id_pos = len(CS.cs)
        # Create ast
...
class CPPro_ifdef(Ast):
    ...
    def __init__(self, line: Line, identifier: str|int) -> None:
        self.line = line
...
class CPPro_if(Ast):
    ...
    def __init__(self, line: Line, expression: str) -> None:
        self.line = line
...
class CPPro_define(Ast):
    ...
    def __init__(self, line: Line, identifier: str, replacement: str) -> None:
        self.line = line
        self.identifier = identifier
...
class CPPro_undef(Ast):
    ...
    def __init__(self, line: Line, identifier: str) -> None:
        self.line = line
        self.identifier = identifier
...
class CPPro_include(Ast):
    ...
    def __init__(self, line: Line, written_include: str, actual_include: str) -> None:
        self.line = line
    ...
    def extract(self, CS: ChangeSetType) -> None:
        ...
        CS.store(m_file_name.get_set(None, self.a_include))
        ...
        CS.store(m_ast.view(
            ...
            108,
            None,
            CS.ref(m_file_name.fnid, REF_POS, ast_id_pos - 1),
        ))
...
class CPPro_line(Ast):
    ...
    def __init__(self, line: Line, lineno: int, filename: str) -> None:
        self.line = line
        self.lineno = lineno
...
class CPPro_error(Ast):
    ...
    def __init__(self, line: Line, error_msg: str) -> None:
        self.line = line
...
class CPPro_pragma(Ast):
    ...
    def __init__(self, line: Line, pragma: str) -> None:
        self.line = line
        self.pragma = pragma
...
class Ast_Type:
    ...
    def __str__(self) -> str:
        ...
        if self.type_style == Ast_Type_Pure:
            pre_result += COLOR.cyan("Ast_Type_Pure(")
        elif self.type_style == Ast_Type_Typedef:
            pre_result += COLOR.cyan("Ast_Type_Typedef(")
        elif self.type_style == Ast_Type_Struct:
            pre_result += COLOR.cyan("Ast_Type_Struct(")
        elif self.type_style == Ast_Type_Function:
            pre_result += COLOR.cyan("Ast_Type_Function(")
        else:
            # self.type_style == Ast_Type_Undefine:
            return COLOR.red("Ast_Type_Undefine()")


        return good_looking_printing(self, pre_result, COLOR.cyan(")"))
...
#
#            if getattr(self, key):
#                to_be_added = f"{COLOR.magenta(key)}:{getattr(self, key)},"
#                if not multi_line_leap:
#                    pre_result += "\n"
#                    result = f"   {result}"
#                    multi_line_leap = True
...
#            result = result.replace("\n", "\n   ")
#
#        return f"{pre_result}{result[1:-1]}{COLOR.cyan(')')}\n"


class Ast_STRUCT_DECL(Ast):
    def __init__(self, line: Line, name: str, children: list[Ast]|None=None) -> None:
        self.line = line
        self.name = name
...
class Ast_UNION_DECL(Ast):
    def __init__(self, line: Line, name: str|None=None, children: list[Ast]|None=None) -> None:
...
class Ast_FUNCTION_DECL(Ast):
    def __init__(self, line: Line, name: str, ast_type: Ast_Type) -> None:
        self.line = line
        self.name = name
...
class Ast_ENUM_DECL(Ast):
    def __init__(self, line: Line, name: str, enumerator_list: tuple[str]) -> None:
        self.line = line
        self.name = name
...
class Ast_MACRO_INSTANTIATION(Ast):
    def __init__(self, line: Line, name: str, filename: str, args) -> None:
        self.line = line
        self.name = name
        self.filename = filename
        self.args = args
...
class Ast_Manager:
    ...
    def ast_type_getter(self, c_children, ast_t=None, bypass_type=None):
        ...
        c_children_type = bypass_type or c_children.type
        ...
        ast_t.debug = tuple(x[x.find(" ") + 1 :] for x in fucked)
        ...
        while (c_children_type.kind == cc.TypeKind.ELABORATED) or (
            c_children_type.kind == cc.TypeKind.POINTER
        ):
        ...
        try:
            ast_t.location_file = (
                f"{c_children_type.get_declaration().extent.start.file}"[
                    len(self.mfdir) :
                ]
            )
        except IndexError:
    ...
    def ast_parse_struct_decl(self, c_children):
        ...
        for member_decl in c_children.get_children():
            ...
            if children:
                if children[-1].__class__.__name__ == "Ast_Struct_STRUCT_DECL":
                    if (
                        f"{member_decl_type.get_declaration().spelling}"
                        == children[-1].name
                    ):
        ...
        return Ast_STRUCT_DECL(
            Line(c_children.extent).cc(self.rawfile), c_children.spelling, children
        )
    ...
    def ast_parse_macro_instantiation(self, c_children):
        ...
        if name in excluded_keywords:
            return None  # SPECIAL OUTPUT

        filename = None
        if c_children.get_definition():
            filename = str(c_children.get_definition().extent.start.file)[
                len(self.mfdir) + 1 :
            ]
    ...
    def Init_Parse(self) -> None:
        ...
        Skipped_ranges = cc.conf.lib.clang_getSkippedRanges(
            translation_unit, translation_unit.get_file(self.fullfilename)
        )
        ...
        if G.OVERRIDE_CPPRO_PRINT and G.OVERRIDE_GLOBAL_C_AST:
            ...
            print(COLOR.green("=======End CPPro Result======="))

        if G.OVERRIDE_GLOBAL_C_AST:
            print(COLOR.green("======PRINT LOOP======"))
            # PRINT LOOP
            for x in self.processing_list:
                print(x)

        def type_of_warning(argx: int) -> str:
            match argx:
                case 0:
                    return "Ignored"
                case 1:
        ...
        if G.OVERRIDE_CINDEX_ERROR_PRINT and G.OVERRIDE_GLOBAL_C_AST:
            print(COLOR.green("=======Cindex Errors======="))
            if self.diag:
                print(COLOR.red("Found Errors:"))
                for diag in self.diag:
                    print(
                        G.string_shortner(
                            f"[{type_of_warning(diag.severity)}] - {diag.spelling} (Line:{diag.location.line} File:{diag.location.file})",
                            "indent",
                        )
                    )
